import{p as v,h as b,k as m,l as f,j as y,r as x,a as $,i as w,m as k}from"./render-home-DkvHZ7K0.js";import{initSlideshows as E}from"./slideshow-B52kY28T.js";import{i as S}from"./scroll-reveal-6DLuizO8.js";import{r as P}from"./render-cards-CN1Xy30p.js";import{a as L,g as C}from"./activity-packages-DNtWlWFj.js";import{initCardTilt as A}from"./card-tilt-hVmWyP2d.js";import{initCardMotion as q}from"./card-motion-CgCTZbYj.js";function r(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}function _(t){return v(t,4)}function T(t,e=3){const a=[],s=new Set([t]);for(const i of b){if(a.length>=e)break;s.has(i.id)||(a.push(i),s.add(i.id))}return a}function I(t,e=3){return t!=null&&t.length?`
    <ul class="dest-page__pills" aria-label="Highlights">
      ${t.slice(0,e).map(a=>`<li>${r(a)}</li>`).join("")}
    </ul>
  `:""}function M(t){var a;return((a=t.descriptions)!=null&&a.length?t.descriptions.filter(Boolean).slice(0,5):f(t.description)).map((s,i)=>i===0?`<p class="dest-page__lead">${r(s)}</p>`:`<p class="dest-page__text">${r(s)}</p>`).join("")}function N(t,e){const a=m(t,{excludeSrc:e,max:4});return a.length?`
  ...
  `:""}function H(t){return t.map(e=>{var a,s;return`
        <article class="dest-card dest-card--b dest-card--teaser dest-card--related" data-reveal>
          ...
          <a class="dest-card__cinema-body dest-card__cinema-body--teaser" href="${y(e)}">
            ${B(e.highlights,2)}
            <span class="dest-card__link">Explore ${r(e.name)} →</span>
          </a>
        </article>
      `}).join("")}function R(t,e){const a=document.querySelector(t);if(!a||!e)return!1;const s=_(e),i=s[0]||e.images[0],n=T(e.id,3);document.title=`${e.fullName} | Matembo Safari & Tours`;const d=document.querySelector('meta[name="description"]');return d&&d.setAttribute("content",e.description),a.innerHTML=`
  ...
  `;const d=i.querySelector(".dest-page__related");d?i.insertBefore(n,d):i.appendChild(n),await P("#experience-packages-grid",{packages:s,viewLabel:"View Safari"}),A(),q()}async function u(){try{x(),$(),w();const t=G(),e=k(t);if(!e){j("#experience-view-root");return}if(!R("#experience-view-root",e))return;await D(t),E(),z(_(e))}catch(t){console.error("Matembo experience view init failed:",t)}finally{S()}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",u):u();
