import"./buttons-r323SqNs.js";/* empty css             */import{safariPackages as h,getSafariPackageById as d}from"./safari-packages-DmAoeZIg.js";import{initSlideshows as y}from"./slideshow-CpJtcc9p.js";import{i as m}from"./scroll-reveal-BPEmY50i.js";const o="2-days-to-tarangire-national-park-ngorongoro-crater";function _(a){return d(a)??d(o)??h[0]}_(o);function e(a){const t=document.createElement("div");return t.textContent=a??"",t.innerHTML}function b(){const a=new URLSearchParams(window.location.search).get("id");return _(a)}function v(a){return a.safariType||a.circuit||"Safari Package"}function p(a,t="",i){return a!=null&&a.length?a.map((s,l)=>`
        <div class="pkg-slideshow__slide ${t} ${l===0?"is-active":""}" data-slide>
          <img src="${e(s.src)}" alt="${e(s.alt)}" width="1200" height="700" loading="${l===0?"eager":"lazy"}" decoding="async" />
        </div>
      `).join(""):`
      <div class="pkg-slideshow__slide ${t} is-active" data-slide>
        <img src="${e(i==null?void 0:i.src)}" alt="${e(i==null?void 0:i.alt)}" width="1200" height="700" loading="eager" decoding="async" />
      </div>
    `}function g(a){const t=Math.max(a,1);return Array.from({length:t},(i,s)=>`<button type="button" class="dest-slideshow__dot ${s===0?"is-active":""}" data-slide-dot aria-label="Slide ${s+1}"></button>`).join("")}function u(a){return a!=null&&a.length?a.map((t,i)=>`
      ...
      `).join(""):'<p class="pkg-section__text">Day-by-day routing is available on enquiry. Share your dates and we will confirm lodges, drives, and park order for this package.</p>'}function n(a,t){return(a??[]).map(i=>`<li class="${t}">${e(i)}</li>`).join("")}function w(a){return(a??[]).map(t=>`
        <div class="pkg-facts__item">
          <span class="pkg-facts__label">${e(t.label)}</span>
          <span class="pkg-facts__value">${e(t.value)}</span>
        </div>
      `).join("")}function $(a){var l;const t=document.querySelector("#package-view-a");if(!t)return;const i=(l=a.gallery)!=null&&l.length?a.gallery:[{src:a.image,alt:a.alt||a.title}],s=a.badge?`<span class="pkg-sidebar-card__badge">${e(a.badge)}</span>`:"";t.innerHTML=`
    <div class="pkg-view-a">
      ...
      <div class="pkg-gallery-compact" data-slideshow data-interval="6000">
        <div class="dest-slideshow__track dest-slideshow__track--mini">
          ${p(i,"dest-slideshow__slide--motion",{src:a.image,alt:a.alt||a.title})}
        </div>
        ...
        <div class="dest-slideshow__dots dest-slideshow__dots--compact">${g(i.length)}</div>
      </div>
  ...
  `}function k(a){var s;const t=document.querySelector("#package-view-b");if(!t)return;const i=(s=a.gallery)!=null&&s.length?a.gallery:[{src:a.image,alt:a.alt||a.title}];t.innerHTML=`
      ...
      <div class="pkg-tabs" data-tabs>
        ...
        <div class="pkg-tabs__panel" data-panel="gallery">
          <div class="pkg-gallery-grid">
            ${i.map(l=>`<figure class="pkg-gallery-grid__item"><img src="${e(l.src)}" alt="${e(l.alt)}" loading="lazy" width="600" height="400" /></figure>`).join("")}
          </div>
        </div>
      </div>
