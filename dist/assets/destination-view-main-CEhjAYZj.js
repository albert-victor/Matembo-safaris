import{j as g,c as y,r as $,a as w,i as p}from"./render-home-XFWJLOTR.js";import{i as x}from"./slideshow-DONQcubt.js";import{i as E}from"./scroll-reveal-DRkjB4s9.js";function s(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}function k(t){return`/destinations/${encodeURIComponent(t.id)}.html`}const m=["serengeti","ngorongoro","kilimanjaro","tarangire","manyara","arusha"],S=["ruaha","nyerere","katavi","kitulo","lake-nyasa","matema-beach","isimila","mikumi"],A=["mikumi","udzungwa","saadani","kilwa-kisiwani","bagamoyo","pangani"],R=["mahale","gombe","rubondo","lake-tanganyika","saanane"],D=["zanzibar","stone-town","jozani-forest","nungwi-beach","pemba","chumbe-island"],T=["mafia","chole-bay","juani-island","misali-island"],L=["pemba","misali-island","fanjove-island","chumbe-island","mnemba-island","thanda-island"],N={southern:S,eastern:A,western:R,northern:m,zanzibar:D,mafia:T,oceanic:L};function P(t,e=3){const a=g(t),i=(a==null?void 0:a.circuit)||"northern",n=N[i]||m,o=y.filter(d=>d.circuit===i),r=[],c=new Set([t]);for(const d of n){if(r.length>=e)break;if(c.has(d))continue;const h=g(d);h&&h.circuit===i&&(r.push(h),c.add(d))}for(const d of o){if(r.length>=e)break;c.has(d.id)||(r.push(d),c.add(d.id))}return r}function v(t){var n;const e=(n=t.gallery)!=null&&n.length?t.gallery:t.images,a=new Set,i=[];for(const o of e)if(!(!(o!=null&&o.src)||a.has(o.src))&&(a.add(o.src),i.push(o),i.length>=12))break;return i}function U(t,e){return t.map((a,i)=>`
  ...
  `:""}function q(t){return t.map(e=>`
        <article class="dest-card dest-card--b dest-card--teaser dest-card--related" data-reveal>
          ...
          <a class="dest-card__cinema-body dest-card__cinema-body--teaser" href="${k(e)}">
            ${f(e.highlights,2,"dest-card__tags")}
            <span class="dest-card__link">Explore ${s(e.name)} →</span>
          </a>
        </article>
      `).join("")}function C(t,e){const a=document.querySelector(t);if(!a||!e)return!1;const i=v(e),n=i[0],o=P(e.id,3);document.title=`${e.fullName} | Matembo Safari & Tours`;const r=document.querySelector('meta[name="description"]');return r&&r.setAttribute("content",e.description),a.innerHTML=`
    <article class="dest-page">
      ...
      <section class="dest-page__related" aria-labelledby="related-title">
        <div class="container">
          ...
          <div class="dest-page__related-grid">${q(o)}</div>
        </div>
      </section>
    </article>
  ...
  `,!0}function I(t){const e=document.querySelector(t);e&&(e.innerHTML=`
